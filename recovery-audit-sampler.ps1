...
<#
.SYNOPSIS
  Stage 2: builds a stratified, size-limited recovery audit sample plan.

.DESCRIPTION
  recovery-audit-sampler.ps1 is Stage 2 of Recovery Audit Toolkit. It consumes
  the master CSV created by recovery-audit-inventory.ps1 and turns a very large
  file inventory into a deterministic, size-limited audit sample plan.

  The sampler is planning-only: it does not open or copy source files. This
  separation allows the expensive source-volume inventory to be created once
  and the sampling strategy to be rerun without rescanning the source.
...
$outDir=Join-Path ([System.IO.Path]::GetDirectoryName($InventoryCsv)) ('recovery-audit-sampler_'+$RunID)
...
$summary.Add('RECOVERY AUDIT SAMPLER')
$summary.Add('======================')
...
Write-Host
Write-Host 'RECOVERY AUDIT SAMPLER - DONE'
Write-Host '================================'
