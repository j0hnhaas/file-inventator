...
.DESCRIPTION
  Version 1.1 is planning-only. It reads the file-inventator master CSV and
  ...
  Each time stratum receives a nominal share of the total byte budget. Inside
  each stratum, type groups receive reserved shares. Unused reserved bytes are
  then filled in priority order, with Desktop files first and newest files
  first. Unused time-stratum bytes roll forward to older strata.
...
$outDir=Join-Path ([System.IO.Path]::GetDirectoryName($InventoryCsv)) ('file-audit-selector-v1.1_'+$RunID)
...
$candidateWriter.WriteLine('FileID;SourceID;FullPath;RelativePath;Extension;SizeBytes;LastWriteTime;TypeGroup;TypeRank;TimeBand;IsDesktop')
...
try{
  ...
  while(-not $parser.EndOfData){
    ...
    $candidateWriter.WriteLine((@((Csv $row[$idx['FileID']]),(Csv $sourceId),(Csv $path),(Csv $row[$idx['RelativePath']]),(Csv $ext),([string]$size),(Csv $row[$idx['LastWriteTime']]),(Csv $typeGroup),([string]$typeRank),(Csv $timeBand),(Csv ([string]$desktop))) -join ';'))
  }
}
finally{
...
[UInt64]$CarryForward=0

try{
  for($i=0;$i -lt $TimeBands.Count;$i++){
    [void](Assert-ReadOnly $DiskNumber $ExpectedSerial)
    $band=$TimeBands[$i]
    [UInt64]$bandBudget=[UInt64]$TimeBudgetBytes[$band]+$CarryForward
    [UInt64]$bandUsed=0
    $bandRows=@($candidates | Where-Object {$_.TimeBand -eq $band})

    if($bandRows.Count -gt 0){
      foreach($typeGroup in $TypeWeights.Keys){
        [UInt64]$reserved=[UInt64][math]::Floor($bandBudget*([double]$TypeWeights[$typeGroup]/100.0))
        if($reserved -eq 0){continue}

        $typed=@($bandRows | Where-Object {$_.TypeGroup -eq $typeGroup} | Sort-Object @{Expression={if($_.IsDesktop -eq 'True'){0}else{1}};Ascending=$true},@{Expression={[datetime]$_.LastWriteTime};Descending=$true},@{Expression={$_.FileID};Ascending=$true})
        if($typed.Count -eq 0){continue}

        $result=Select-RowsWithinBudget -Rows $typed -Budget $reserved -SelectedIds $SelectedIds -Stage 'TYPE_RESERVED' -Band $band -PlanWriter $planWriter
        $bandUsed+=[UInt64]$result.Bytes
      }

      [UInt64]$remaining=if($bandBudget -gt $bandUsed){$bandBudget-$bandUsed}else{0}
      if($remaining -gt 0){
        $fillRows=@($bandRows | Sort-Object @{Expression={[int]$_.TypeRank};Ascending=$true},@{Expression={if($_.IsDesktop -eq 'True'){0}else{1}};Ascending=$true},@{Expression={[datetime]$_.LastWriteTime};Descending=$true},@{Expression={$_.FileID};Ascending=$true})
        $result=Select-RowsWithinBudget -Rows $fillRows -Budget $remaining -SelectedIds $SelectedIds -Stage 'PRIORITY_FILL' -Band $band -PlanWriter $planWriter
        $bandUsed+=[UInt64]$result.Bytes
      }
    }

    $CarryForward=if($bandBudget -gt $bandUsed){$bandBudget-$bandUsed}else{0}
    Write-Progress -Activity 'Pass 2/2 - Building stratified sample plan' -Status ('{0:N0} files | {1:N3} decimal GB' -f $script:SelectedFiles,($script:SelectedBytes/1000000000)) -CurrentOperation $band -PercentComplete ((($i+1)/[double]$TimeBands.Count)*100)
  }
}
finally{
  $planWriter.Flush()
...
$summary.Add('FILE AUDIT SELECTOR V1.1')
$summary.Add('========================')
...
$summary.Add('')
$summary.Add('TYPE GROUP SHARES AND RESULTS')
...
foreach($group in $TypeWeights.Keys){
  ...
  $summary.Add("$group.ReservedPercentPerBand=$($TypeWeights[$group])")
...
Write-Host
Write-Host 'FILE AUDIT SELECTOR V1.1 - DONE'
Write-Host '================================'
