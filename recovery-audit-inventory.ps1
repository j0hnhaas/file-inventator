...
<#
.SYNOPSIS
  Stage 1: read-only, ACL-aware inventory of a mounted Windows volume.

.DESCRIPTION
  recovery-audit-inventory.ps1 is Stage 1 of Recovery Audit Toolkit. It builds
  the master file inventory consumed by recovery-audit-sampler.ps1.

  It inventories all regular files that Robocopy can enumerate with backup
  rights. It is intended for a mounted source disk that Windows already
...
.SCOPE
  The inventory stage does not recover deleted files, inspect unallocated
  space, carve files, inspect other unmounted/RAW partitions, or calculate
  content hashes.

.EXAMPLE
  .\recovery-audit-inventory.ps1 -SourceRoot 'X:\' -ExpectedSerial 'SERIAL_NUMBER'
#>
...
$ScriptName = 'recovery-audit-inventory'
$ScriptVersion = '1.0.2'
$RunStarted = Get-Date
...
function Write-Manifest([string]$Status,[string]$Validation,[hashtable]$Extra=@{}) {
  ...
  $lines.Add('RECOVERY AUDIT INVENTORY - MANIFEST')
  $lines.Add('========================================')
  $lines.Add('')
...
Write-Host
Write-Host 'RECOVERY AUDIT INVENTORY'
Write-Host '========================'
